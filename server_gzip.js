...
http.createServer(function (request, response) {
  ...
  fs.exists(filename, function (exists) {
    ...
    fs.readFile(filename, 'binary', function (err, file) {
      ...
      if (acceptEncoding.match(/\bdeflate\b/) || acceptEncoding.match(/\bgzip\b/)) {
        response.writeHead(200, {'Content-encoding': 'gzip', 'Content-Type': contentTypeMap[extension] || 'text/plain'});
